...
import { Plus, Search, Filter, MoreVertical, Monitor, Clock, Users } from "lucide-react"
...
export default function RDPConnectionsPage() {
  const router = useRouter()
  const [isNewConnectionOpen, setIsNewConnectionOpen] = useState(false)

  const handleConnect = (connectionId: number) => {
    router.push(`/session/rdp-${connectionId}`)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold mb-2 text-white">RDP Connections</h1>
          <p className="text-gray-300">Manage your Remote Desktop Protocol connections</p>
        </div>
        <Dialog open={isNewConnectionOpen} onOpenChange={setIsNewConnectionOpen}>
          ...
          <DialogContent className="bg-[#1a1a2e] border-[rgba(91,194,231,0.2)] max-w-2xl">
            <DialogHeader>
              <DialogTitle className="text-white">Add New RDP Connection</DialogTitle>
            </DialogHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-[#1a1a2e] border-[rgba(91,194,231,0.2)]">
          <p className="text-sm text-gray-400 mb-1">Total RDP</p>
          <p className="text-2xl font-bold text-white">{rdpConnections.length}</p>
        </Card>
        <Card className="p-4 bg-[#1a1a2e] border-[rgba(91,194,231,0.2)]">
          <p className="text-sm text-gray-400 mb-1">Connected</p>
          <p className="text-2xl font-bold text-[#00ff88]">
            {rdpConnections.filter((c) => c.status === "connected").length}
          </p>
        </Card>
        <Card className="p-4 bg-[#1a1a2e] border-[rgba(91,194,231,0.2)]">
          <p className="text-sm text-gray-400 mb-1">Total Sessions</p>
          <p className="text-2xl font-bold text-[#5bc2e7]">
            {rdpConnections.reduce((sum, c) => sum + c.sessions, 0)}
          </p>
        </Card>
        <Card className="p-4 bg-[#1a1a2e] border-[rgba(91,194,231,0.2)]">
      ...
      <Card className="p-4 bg-[#1a1a2e] border-[rgba(91,194,231,0.2)]">
        <div className="flex items-center gap-4">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder="Search RDP connections..."
