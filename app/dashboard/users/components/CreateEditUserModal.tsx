...
interface CreateEditUserModalProps {
  ...
  currentUserOrgId?: string; // Optional: if provided, organization will be auto-assigned
}
...
export default function CreateEditUserModal({
  ...
  const onSubmit = (data: CreateUserFormData | UpdateUserFormData) => {
    // If currentUserOrgId is provided, use it instead of the form value
    const organizationId = currentUserOrgId || data.organization_id;

    if (!isEditing) {
      mutate({ ...data, organization_id: organizationId } as CreateUserFormData);
      return;
    }
    updateMutate({
      id: editingUser?.id ?? "",
  ...
  useEffect(() => {
    ...
    // Set organization_id from currentUserOrgId if provided
  ...
  }, [isOpen, isEditing, editingUser, reset, currentUserOrgId]);
